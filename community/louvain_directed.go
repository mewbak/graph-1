...
func Qdir(g graph.Directed, communities [][]graph.Node, resolution float64) float64 {
	...
	if communities == nil {
		...
		for _, u := range nodes {
			kU := k[u.ID()]
			q += weight(u, u) - resolution*kU.in*kU.out/m
		}
		return q / m
	}
	...
	for _, c := range communities {
		for _, u := range c {
			...
			for _, v := range c {
				kV := k[v.ID()]
				q += weight(u, v) - resolution*kU.in*kV.out/m
			}
		}
	}
	return q / m
...
func (l *directedLocalMover) deltaQ(n graph.Node) (deltaQ float64, dst int, src commIdx) {
	...
	for connected.TakeMin(&i) {
		...
		case removal:
			...
			dQremove = 2*(k_aC.in /*^𝛼*/ -a_aa) + 2*(k_aC.out /*^𝛼*/ -a_aa) -
				2*gamma*k_a.in*(sigma_totC.in /*^𝛼*/ -k_a.in)/m -
				2*gamma*k_a.out*(sigma_totC.out /*^𝛼*/ -k_a.out)/m

		default:
			// Otherwise calculate the change due to an addition
			// to c and retain if it is the current best.
			dQ := 2*k_aC.in /*^𝛽*/ + 2*k_aC.out /*^𝛽*/ -
				2*gamma*k_a.out*sigma_totC.in /*^𝛽*/ /m -
				2*gamma*k_a.in*sigma_totC.out /*^𝛽*/ /m
